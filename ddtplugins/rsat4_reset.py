...
plugin_name = _("RSAT4 AIRBAG Reset")
...
class Virginizer(widgets.QDialog):
    ...
    def check_virgin_status(self):
        ...
        crash_response = crash_reset_request.send_request(
            {}, "62 02 04 00 00 00 00 00 00 00 00 00 00 00 00")
            ...
            if u"crash detected" in crash_response:
                ...
                if crash == u'crash detected':
                    self.status_check.setText(
                        _("<font color='red'>CRASH DETECTED</font>"))
                    return
                if crash == u'no crash detected':
                    self.status_check.setText(
                        _("<font color='green'>NO CRASH DETECTED</font>"))
                    return

        self.status_check.setText(
            _("<font color='orange'>UNEXPECTED RESPONSE</font>"))
    ...
    def start_diag_session(self):
        ...
        sds_stream = " ".join(sds_request.build_data_stream(
            {u'Session Name': u'extendedDiagnosticSession'}))
    ...
    def reset_ecu(self):
        ...
        request_response = reset_request.send_request(
            {u'CLEDEV For reset crash': '13041976'})

        if request_response is not None:
            self.status_check.setText(
                _("<font color='green'>CLEAR EXECUTED</font>"))
        else:
            self.status_check.setText(
                _("<font color='red'>CLEAR FAILED</font>"))
